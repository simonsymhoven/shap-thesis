...
\nomenclature{\( y^{(i)} \)}{Zielgröße, abhängige Variable}
\nomenclature{\( x^{(i)}_{j} \)}{Wert der unabhängigen Variablen \( j \) für die Beobachtung \( i \)}
\nomenclature{\( \beta_j \)}{Koeffizient oder Gewichtung des Prädiktors \( j \) im linearen Modell}
\nomenclature{\( \epsilon^{(i)} \)}{Residuum für die Beobachtung \( i \)}
