...
\section{Herkömmliche Interpretation}
...
Die prognostizierte Laufzeit für die Beobachtung $x^{(0)}$ beträgt folglich $e^{3.257} \approx 25.97$ MPa.
...
Es fällt auf, dass die Reihenfolge der Merkmalsrelevanz im Vergleich zu Abbildung \ref{pic:permutation} gleich ist.
Die Interpretation ist jedoch eine andere. Die Permutation Feature Importance sagt lediglich aus, wie sehr sich der mittlere
quadratische Fehler des Modell verändert, wenn das entsprechende Merkmal entfernt wird. Der SHAP Bar Plot hingegegen zeigt die
durchschnittliche Auswirkung auf das Modell, also die Veränderung der abhängigen Variable über alle Beobachtungen.
