...
\section{Monte Carlo Integration zur Schätzung von SHAP-Werten}
...
und die SHAP-Werte $\hat{\varphi}_{j}^{(i)}$ analog zu Gleichung \ref{eq:shap-eq}:
...
Gleichung \ref{eq:monte-shap-eq} löst zwar nun das Problem der Integration, setzt allerdings nach wie vor 
vorraus, sämtliche Koalitionen zu berechnen. 